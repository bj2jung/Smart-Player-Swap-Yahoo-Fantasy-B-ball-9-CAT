...
if (
  Number.isInteger(Number(currentPageURL.split("/")[4])) &&
  Number.isInteger(Number(currentPageURL.split("/")[5])) &&
  !currentPageURL.split("/")[6]
) {
  let teamID = currentPageURL.split("/")[5];
  localStorage.setItem("teamID", teamID);
}

//run the program only on "players" tab of website
if (currentPageURL.split("/")[5].includes("players")) {
...
populate currentWeekGameScheduleArray with games that occur in the current week (Monday thru Sunday) and nextWeekGameScheduleArray for following week  */
  ...
  async function getGameSchedule() {
    ...
    gameScheduleCSV
      ...
      .then(() =>
        populateNextWeekGameScheduleArray(getTimeStampOfCurrentWeek())
      );
  }
  ...
  function getMyRoster() {
    return fetch(
      `https://basketball.fantasysports.yahoo.com/nba/${leagueID}/${teamID}`
    )
...
- projected stats per day https://basketball.fantasysports.yahoo.com/nba/44190/4/team?&date=2018-12-12&stat1=P&stat2=P
  */
  ...
  const myRosterStatsUrls = [
    `https://basketball.fantasysports.yahoo.com/nba/${leagueID}/${teamID}?stat1=AS&stat2=AS_2018`,
    `https://basketball.fantasysports.yahoo.com/nba/${leagueID}/${teamID}?stat1=AS&stat2=AS_2017`,
    `https://basketball.fantasysports.yahoo.com/nba/${leagueID}/${teamID}?stat1=AS&stat2=AL7`,
    `https://basketball.fantasysports.yahoo.com/nba/${leagueID}/${teamID}?stat1=AS&stat2=AL14`
  ];

  function updateMyRosterStatsObject() {
    let j = 0;
    asyncForEach(Object.keys(myRosterStatsObject), async key => {
      myRosterStatsObject[key] = await collectMyRosterStats(
        myRosterStatsUrls[j]
      );
      j++;
    });
